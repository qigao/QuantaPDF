...
#define QUANTAPDF_COMPOSER_FONT_OPTIONS_V1_MIN_SIZE (sizeof(size_t))
#define QUANTAPDF_COMPOSER_FONT_OPTIONS_V1_SIZE \
    (sizeof(quantapdf_composer_font_options))

typedef enum quantapdf_composer_text_run_font_kind {
    QUANTAPDF_COMPOSER_TEXT_RUN_BASE14 = 1,
    QUANTAPDF_COMPOSER_TEXT_RUN_EMBEDDED = 2
} quantapdf_composer_text_run_font_kind;

typedef struct quantapdf_composer_text_run_options {
    size_t struct_size;
    quantapdf_composer_text_run_font_kind font_kind;
    quantapdf_composer_font base14_font;
    quantapdf_composer_font_id embedded_font_id;
    float font_size;
    uint32_t argb;
    float line_height_multiplier;
    quantapdf_composer_text_alignment alignment;
    int wrap;
} quantapdf_composer_text_run_options;

#define QUANTAPDF_COMPOSER_TEXT_RUN_OPTIONS_V1_MIN_SIZE \
    (offsetof(quantapdf_composer_text_run_options, wrap) + sizeof(int))
#define QUANTAPDF_COMPOSER_TEXT_RUN_OPTIONS_V1_SIZE \
    (sizeof(quantapdf_composer_text_run_options))
...
QUANTAPDF_API quantapdf_status quantapdf_composer_draw_text_run(
    quantapdf_composer *composer,
    size_t page_index,
    const char *text_utf8,
    const quantapdf_rect *bounds,
    const quantapdf_composer_text_run_options *options);
